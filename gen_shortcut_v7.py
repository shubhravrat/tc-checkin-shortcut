#!/usr/bin/env python3
"""
Generate TC-Daily-Checkin-v7.shortcut.

v7 fixes the choosefrommenu/setvariable serialization against REAL evidence
instead of doc snippets, after two straight on-device test failures (empty
clipboard, then Paste-shows-but-empty-content). Decoded two actual working
iOS-exported .shortcut binaries pulled from GitHub
(MediaGrabber/iOS-Workflow "Media Grabber.shortcut" and Faylixe/covidcert
shortcuts/*.shortcut, both containing real choosefrommenu + setvariable +
Variable-in-text patterns) and matched their exact shape byte-for-byte:

1. choosefrommenu actions in mode 0 (menu def) and mode 1 (case) NEVER carry
   a `UUID` key in real exports -- only the mode 2 (End Menu) action does.
   v6 put a UUID on all four menu-block actions; that extraneous key is the
   most likely cause of the still-empty clipboard content after the v6
   Set-Variable WFInput fix.
2. `is.workflow.actions.setvariable` actions also never carry a `UUID` in
   real exports (only `WFVariableName` + `WFInput`).
3. Referencing a named variable directly inside the main Text action's
   attachmentsByRange as `{"Type": "Variable", "VariableName": ...}` (no
   wrapper) IS the real, proven mechanism -- confirmed by the covidcert
   export's final JSON-building Text action, which does exactly this for
   nine different variables. v6's detour through an extra Get Variable
   action per field was unnecessary; removed here.

Everything else (Health Sum for Water/Steps, Day reusing Formatted Date,
typed Sleep/Stress/Supplements, Open App -> Fittr instead of Share) is
unchanged from v6.
"""
import plistlib
import uuid
...
def yes_no_menu(prompt, variable_name, yes_label="Yes ✅", no_label="No ❌"):
    ...
    Shape matched against real iOS-exported plists: the menu-def (mode 0)
    and case (mode 1) actions carry no UUID; only End Menu (mode 2) does.
    Set Variable also carries no UUID -- its output is never referenced by
    UUID, only by name.
...
def validate(path, expected_action_count):
    ...
    for a in actions:
        ...
        assert wf_input is not None, f"a setvariable action has no WFInput"
    ...
    # WFMenuItemTitle order matches WFMenuItems order. Modes 0/1 carry no UUID
    # (matches real iOS exports); only mode 2 does.
    ...
    for i in range(0, len(menu_actions), 4):
        ...
        assert "UUID" not in block[0]["WFWorkflowActionParameters"], f"menu def at index {i} should not have UUID"
        assert "UUID" not in block[1]["WFWorkflowActionParameters"], f"case at index {i+1} should not have UUID"
        assert "UUID" not in block[2]["WFWorkflowActionParameters"], f"case at index {i+2} should not have UUID"
        assert "UUID" in block[3]["WFWorkflowActionParameters"], f"end menu at index {i+3} should have UUID"
...
if __name__ == "__main__":
    wf = build()
    out_path = "TC-Daily-Checkin-v7.shortcut"
